...
app.post("/api/insert", (req: { body: { id: number; taskName: string; description: string; createdBy: string; createdOn: number; }; }, res: any) => {
  ...
  const sqlInsert = 'INSERT INTO todoinfo (id, taskName, description, createdBy, createdOn) VALUES (?,?,?,?,?)';
...
app.get('/api/get', (req: any, res: any) => {
  const sqlSelect = 'SELECT * FROM todoinfo';
...
app.delete("/api/delete/:id", (req: { body: { id: any } }, res: any) => {
  const id = req.body.id
  const sqlDelete = "DELETE FROM todoinfo WHERE id=?";
...
app.put("/api/update", (req: { body: { id: any; taskName: any; description: any; createdBy: any; createdOn: any; }; },res: any)=>{
  ...
  const sqlUpdate = "UPDATE todoinfo SET taskName=?,description=?,createdBy=?,createdOn=? WHERE id=?";
